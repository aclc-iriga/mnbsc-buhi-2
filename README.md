...
Tabulation System for Mutya nin Bagong Santa Clara (Buhi, Camarines Sur)
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/mnbsc-buhi-2`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `mnbsc-buhi-2` and import [mnbsc-buhi-2.sql](mnbsc-buhi-2.sql) into it.

6. Open the terminal and navigate to the project directory **mnbsc-buhi-2**.
...
9. Open your web browser and access <http://localhost:5202/mnbsc-buhi-2/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-mnbsc-buhi-2` and import [mnbsc-buhi-2.sql](mnbsc-buhi-2.sql) into it.
...
2. Access the application by visiting `http://[host_name]/mnbsc-buhi-2`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/mnbsc-buhi-2>
     - <http://192.168.1.99/mnbsc-buhi-2>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/mnbsc-buhi-2/app>.
